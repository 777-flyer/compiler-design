...
void yyerror(char *s)
{
	outlog<<"Error at line "<<line_num<<": "<<s<<endl<<endl;
}

%}

/* keywords that are tokenized but not used by any grammar rule below   */
/* (the language subset for this lab does not use switch/break/do/etc.) */
%token IF FOR WHILE DO INT FLOAT DOUBLE VOID CHAR
%token SWITCH BREAK DEFAULT GOTO RETURN CONTINUE CASE PRINTLN
...
/* classic dangling-else fix: giving ELSE higher precedence than the    */
/* "if without else" rule tells yacc to always shift ELSE instead of    */
/* reducing, which makes else bind to the nearest unmatched if and      */
/* removes the shift/reduce conflict that this grammar would otherwise  */
/* report.                                                              */
...
int main(int c, char *v[])
{
	if(c != 2)
	{
		cout<<"Please give the input file name as a command line argument."<<endl;
		return 0;
	}
	...
	outlog<<endl<<"Total lines: "<<line_num<<endl;
